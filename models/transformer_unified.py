...
class Transformer(nn.Module):
    def __init__(
        ...
        if "mq" in self.args.tasks.names:
            ...
            if self.args.model.mq.use_class_text_embeddings:
                # external class-text embeddings
                self.mq_resizer_class_text_embeddings = FeatureResizer(
                    input_feat_size=768,
                    output_feat_size=d_model,
                    dropout=0.1,
                )
                p_dict_idx_to_encoded_moment_class = Path(
                    "/private/home/raghavgoyal/code/episodic-memory-net/TubeDETR_3c32cc9/assets/") / "dict_idx_to_encoded_moment_class.pt"
                assert p_dict_idx_to_encoded_moment_class.is_file()
                self.dict_idx_to_encoded_moment_class = torch.load(p_dict_idx_to_encoded_moment_class)
            elif self.args.model.mq.use_text_encoder:
                ...
                # one-hot mode
                self.mq_class_one_hot_embeddor = FeatureResizer(
                    input_feat_size=len(self.mq_class_to_idx) - 1,
                    output_feat_size=d_model,
                    dropout=0.1,
                )
        # NLQ
        if "nlq" in self.args.tasks.names:
            if self.args.model.nlq.use_sentence_text_embeddings:
                ...
                print("[NLQ] Loading sentence embeddings for train and val only !!")
                self.dict_nlq_sentences_to_embeddings = {}
                for split_name in ['train', 'val']:
                    p_dict_nlq_sentences_to_embeddings = (Path(
                        "/private/home/raghavgoyal/code/episodic-memory-net/TubeDETR_3c32cc9/assets/")
                        / f"dict_nlq_sentences_to_embeddings_{split_name}.pt")
                    assert p_dict_nlq_sentences_to_embeddings.is_file()
                    print(f"[NLQ] Loading sentence embeddings from {p_dict_nlq_sentences_to_embeddings}")
                    self.dict_nlq_sentences_to_embeddings.update(torch.load(p_dict_nlq_sentences_to_embeddings))

                print(f"[NLQ] Total size of embedding dictionary: {len(self.dict_nlq_sentences_to_embeddings)}")

                self.nlq_resizer_class_text_embeddings = FeatureResizer(
                    input_feat_size=768,
                    output_feat_size=d_model,
                    dropout=0.1,
                )
            elif self.args.model.nlq.use_clip_text_encoder:
                ...
                self.expander_dropout = 0.1
                self.resizer_clip = FeatureResizer(
                    input_feat_size=512,
                    output_feat_size=d_model,
                    dropout=self.expander_dropout,
                )
    ...
    def forward(
        ...
        **kwargs
        ...
        if encode_and_save:
            ...
            if task_name == "mq":
                if self.args.model.mq.use_class_text_embeddings:
                    # use pre-extracted class text embeddings
                    assert len(text) == 1
                    encoded_text = self.dict_idx_to_encoded_moment_class[self.mq_class_to_idx[text[0]]]
                    encoded_text = encoded_text.to(src.device)

                    text_memory = encoded_text.transpose(0, 1)
                    # Invert attention mask that we get from huggingface because its the opposite in pytorch transformer
                    text_attention_mask = torch.zeros(
                        (1, text_memory.shape[0]), dtype=torch.bool).to(device)
                    tokenized = None

                    # Resize the encoder hidden states to be of the same d_model as the decoder
                    text_memory_resized = self.mq_resizer_class_text_embeddings(text_memory)

                    # encode caption once per video and repeat each caption X times where X is the number of clips in the video
                    n_repeat = t if (not stride) else math.ceil(t / stride)
                    assert (
                        n_repeat
                    ...
                    assert len(text) == 1
                    # if self.args.debug: import ipdb; ipdb.set_trace()
                    text_cleaned = text[0].replace('_', ' ').replace('"', '')
                    text_question = f"When did I {text_cleaned}?"

                    tokenized = self.tokenizer.batch_encode_plus(
                        [text_question], padding="longest", return_tensors="pt"
                    ).to(device)
                    ...
                    # use one-hot class embeddings
                    assert len(text) == 1
                    class_idx = F.one_hot(
                        torch.tensor(self.mq_class_to_idx[text[0]] - 1),
                        num_classes=len(self.mq_class_to_idx) - 1
                    )
                    class_idx = class_idx.float().to(device)
                    class_embedding = self.mq_class_one_hot_embeddor(class_idx)

                    # import ipdb; ipdb.set_trace()
                    text_memory_resized = class_embedding[None, None, ...]
                    text_attention_mask = torch.zeros(
                        (1, text_memory_resized.shape[0]), dtype=torch.bool).to(device)
                    tokenized = None
                ...
                if self.args.model.nlq.use_sentence_text_embeddings:
                    # use pre-extracted class text embeddings
                    assert len(text) == 1
                    encoded_text = self.dict_nlq_sentences_to_embeddings[kwargs["key_for_nlq_sentence_embedding"]]
                    encoded_text = encoded_text.to(src.device)
                    ...
                    # Resize the encoder hidden states to be of the same d_model as the decoder
                    text_memory_resized = self.nlq_resizer_class_text_embeddings(text_memory)
                    ...
                    encoded_text = self.text_encoder.encode_text(tokenized).float()
                    encoded_text = encoded_text / encoded_text.norm(dim=1, keepdim=True)

                    text_memory = encoded_text[None]

                    text_attention_mask = torch.zeros(1, 1).bool().to(device)

                    text_memory_resized = self.resizer_clip(text_memory)
                ...
                if kwargs["vq2d_use_object_text"]:
                    ...
                    text_memory_resized = kwargs["src_reference_crop"].flatten(2).permute(2, 0, 1)

                    # optionally use projection layer
                    if self.args.model.vq2d.use_projection_layer:
                        text_memory_resized = self.resizer_vq2d(text_memory_resized)
            ...
            if self.fast:
                if (
                    ...
                    time_embed = self.time_embed[task_name](t)
            ...
            if stride:  # temporal replication
                ...
                for i_dur, dur in enumerate(durations):
                    for i_clip in range(n_clips):
                        clip_dur = min(stride, t - i_clip * stride)
                        pad_img_memory[
                            :,
                            i_dur,
                            i_clip * stride: i_clip * stride + clip_dur,
                        ] = (
