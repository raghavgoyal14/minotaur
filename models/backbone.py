...
class IntermediateLayerGetterCustom(IntermediateLayerGetter):
    ...
    def forward(self, x, encode_reference=False):
        out = OrderedDict()
        for name, module in self.items():

            # to get around self containing `conv_mask``
            if name == "conv_mask":
                continue

            if name == "conv1" and encode_reference:
                # import ipdb; ipdb.set_trace()
                x = self.conv1(x[:, :3]) + self.conv_mask.conv1_m(x[:, 3][:, None, ...])
                continue
...
class BackboneBase(nn.Module):
    def __init__(
        ...
        #     and self.args.model.vost.mask_at_full_image_level.enable
        #     and self.args.model.vost.mask_at_full_image_level.type == "image_concat_level"
        # ):
        #     conv_mask = nn.ModuleDict({
        #         "conv1_m": nn.Conv2d(1, backbone.conv1.out_channels,
        ...
        #         )
        #     })
        #     self.body = IntermediateLayerGetterCustom(backbone, return_layers=return_layers, conv_mask=conv_mask)
    ...
    def forward(self, tensor_list, flop_count_mode=False, encode_reference=False):
        # if "vost" in self.args.tasks.names:
        #     if (encode_reference
        #         and self.args.model.vost.mask_at_full_image_level.enable
        #         and self.args.model.vost.mask_at_full_image_level.type == "image_concat_level"
        #     ):
        #         return self.forward_encode_reference(tensor_list)
    ...
    # def forward_encode_reference(self, tensor_list):
    #     # if self.args.debug: import ipdb; ipdb.set_trace()
    #     xs = self.body(tensor_list.tensors, encode_reference=True)
    #     out = OrderedDict()
    #     for name, x in xs.items():
    #         mask = F.interpolate(
    #             tensor_list.mask[None].float(), size=x.shape[-2:]
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list, no_spatial_feature_map=False, encode_reference=False):
        if self[0].__class__.__name__ == "TimmBackbone":
            xs = self[0](tensor_list)
        else:
            xs = self[0](tensor_list, self.flop_count_mode, encode_reference)
